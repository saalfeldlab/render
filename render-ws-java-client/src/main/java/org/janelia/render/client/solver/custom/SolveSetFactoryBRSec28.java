...
public class SolveSetFactoryBRSec28 extends SolveSetFactory
{
			...
			final double defaultDynamicLambdaFactor )
	{
		...
		//3799-4413, 5124-5573, 12059-12916, 20348-21153, 23376-23907, 25382-25892, 32426-32831
		for ( int i = 3799; i <= 4413; ++i )
			this.additionalIssues.put( i, "problem" );

		for ( int i = 5124; i <= 5573; ++i )
			this.additionalIssues.put( i, "problem" );

		for ( int i = 12059; i <= 12916; ++i )
			this.additionalIssues.put( i, "problem" );

		for ( int i = 20348; i <= 21153; ++i )
			this.additionalIssues.put( i, "problem" );

		for ( int i = 23376; i <= 23907; ++i )
			this.additionalIssues.put( i, "problem" );

		for ( int i = 25382; i <= 25892; ++i )
			this.additionalIssues.put( i, "problem" );

		for ( int i = 32426; i <= 32831; ++i )
			this.additionalIssues.put( i, "problem" );
	}
	...
	public SolveSet defineSolveSet( final int minZ, final int maxZ, final int blockSize, final int minBlockSize, final Map<Integer, String> zToGroupIdMap )
	{
		...
		for ( final SetInit initSet : initSets )
		{
			...
			// allow translation stitching only
			stitchingModel = ((InterpolatedAffineModel2D) stitchingModel ).copy();
