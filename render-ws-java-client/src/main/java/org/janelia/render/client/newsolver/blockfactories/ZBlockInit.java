...
class ZBlockInit {
	...
	/**
	 * Implementation that balances blocksize using the average block size.
	 *
	...
	public static List<ZBlockInit> defineBlockLayout(final int minZ, final int maxZ, final int blockSize, final int minBlockSize) {
		...
		if (avgBlock < minBlockSize)
			throw new RuntimeException("average blocksize=" + avgBlock + " given desired blocksize=" + blockSize
											   + ", which is smaller than minBlockSize=" + minBlockSize
											   + " (either increase blocksize - we're not trying that - or reduce minBlockSize). stopping.");
