...
/**
 * Stateless rendering pipeline shared by the {@link TileRenderer} implementations. It splits a tile
 * render into its two natural halves:
 * <ul>
 ...
 * </ul>
 * Neither method caches; the concrete {@link TileRenderer}s decide which half's result to memoize.
 */
class TileRasterizer {
	 ...
	 */
	TileRasterizer(final int numCoefficients, final double scale, final int meshResolution) {
	...
	DownsampledSource loadSource(final TileSpec patch) {
		...
		// Get the entire image at full scale; the per-tile caches in the renderers already dedup loads
