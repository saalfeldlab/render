 ...
 * <ul>
 *     <li>{@link #loadSource} &ndash; the pair-independent part: load the full-resolution source,
 *         apply the tile's filter and downsample to the mipmap level;</li>
 *     <li>{@link #renderBox} &ndash; the box-dependent part: mesh the (downsampled) source into a
 *         requested world-space box and convert to the intensity/weight/label rasters.</li>
 * </ul>
 * Subclasses implement {@link #render} and decide which of these results to cache and reuse across
 * calls; see the concrete implementations for their respective tradeoffs.
 */
abstract class TileRenderer {

	protected final int numCoefficients;
	protected final double scale;
	protected final int meshResolution;

	/**
	 * @param numCoefficients number of coefficients in the intensity model
	 * @param scale render scale
	 * @param meshResolution resolution of the mesh used to render a box
	 */
	TileRenderer(final int numCoefficients, final double scale, final int meshResolution) {
	 ...
	 * and downsample to the mipmap level.
	 */
	DownsampledSource loadSource(final TileSpec patch) {

		// Get the entire image at full scale; the per-tile caches in the subclasses already dedup loads
		// across pairs, so the full-resolution image processor cache would add nothing but memory pressure
	...
	/**
	 * Estimate the average scale of the tile's transforms and pick the corresponding mipmap level.
	 * The box offset added in {@link #renderBox} is a pure translation, which does not affect the
	 * scale of the fitted {@link SimilarityModel2D}, so the mipmap level is the same for every box and
	 * is computed here once (with zero offset).
	 */
	...
	/**
	 * Box-dependent part of the render: mesh the (downsampled) source into the requested world-space
	 * box and convert intensities and weights to [0, 1]. Returns freshly allocated rasters sized
	...
	RenderedRegion renderBox(final TileSpec patch, final DownsampledSource source, final Rectangle box) {
		...
		final FloatProcessor targetImage = new FloatProcessor(w, h);
		final FloatProcessor targetWeight = new FloatProcessor(w, h);
		final ColorProcessor targetCoefficients = new ColorProcessor(w, h);
		mapBox(patch, source, box.x, box.y, targetImage, targetWeight, targetCoefficients);

		// wrap the just-rendered pixel arrays as image views (zero-copy)
		final RandomAccessibleInterval<FloatType> imageView = ArrayImgs.floats((float[]) targetImage.getPixels(), w, h);
	...
	/**
	 * Mesh the source into the target box at world-space top-left {@code (x, y)} and write intensities
	 * [0, 255] &rarr; [0, 1] and weights [0, 255] &rarr; [0, 1] into the supplied target processors,
	 * which define the box dimensions.
	 */
	private void mapBox(final TileSpec patch,
