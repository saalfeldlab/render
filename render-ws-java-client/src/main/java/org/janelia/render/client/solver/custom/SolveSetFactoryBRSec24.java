...
public class SolveSetFactoryBRSec24 extends SolveSetFactory
{
	...
	public SolveSet defineSolveSet( final int minZ, final int maxZ, final int setSize, final Map<Integer, String> zToGroupIdMap )
	{
		...
		for ( int i = 0; i < numSetsLeft; ++i )
		{
			...
			// stitching-first issue when going from two tiles to one tile, no stitching-first
			if ( setMaxZ >= 57300 && setMinZ <= 57350 )
			{
				minStitchingInliers = 100000;
			}
