...
public class SolveItem implements Serializable
{
	...
	private HashMap<Tile<?>, String > tileToIdMap = new HashMap<>();

	// stitching-related (local)

	// contains the model as after local stitching (tmp)
	private HashMap<String, AffineModel2D> idToStitchingModel = new HashMap<>();
	...
	public ImagePlus visualizeInput( final double scale )
	{
		try
		{
			final ImagePlus imp = SolveTools.render( idToPreviousModel, idToTileSpec, 0.15 );
			imp.setTitle( "input" );
			return imp;
		}
	...
	public ImagePlus visualizeAligned( final double scale )
	{
		try
		{
			final ImagePlus imp = SolveTools.render( idToNewModel, idToTileSpec, 0.15 );
	...
	{
		final SolveItem s = new SolveItem( 100, 102, new RunParameters() );
