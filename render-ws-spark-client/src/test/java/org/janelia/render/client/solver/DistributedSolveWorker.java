...
public class DistributedSolveWorker
{
	...
	protected void assembleMatchData() throws IOException
	{
		...
		for ( final Tuple2< String, Double > pGroupPair : runParams.pGroupList )
		{
			if ( pGroupPair._2().doubleValue() < inputSolveItem.minZ() || pGroupPair._2().doubleValue() > inputSolveItem.maxZ() )
				continue;

			final String pGroupId = pGroupPair._1();
				...
				if ( set.size() > 1 )
				{
					...
					if ( visualizeZSection == z )
					{
						try
						{
							...
							new ImageJ();
							final ImagePlus imp1 = SolveTools.render( models, solveItem.idToTileSpec(), 0.15 );
							imp1.setTitle( "z=" + z );
						}
	...
	protected static List< PointMatch > duplicate( final List< PointMatch > pms )
	{
