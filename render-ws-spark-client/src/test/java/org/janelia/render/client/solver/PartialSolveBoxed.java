...
public class PartialSolveBoxed< B extends Model< B > & Affine2D< B > >
{
	...
	protected void run() throws IOException, ExecutionException, InterruptedException, NoninvertibleModelException
	{
		...
		for (final String pGroupId : runParams.pGroupList.stream().map(Tuple2::_1).collect( Collectors.toList() ) )
		{
		...
		for ( final String tileId : tileIds )
		{
			...
			if ( z <= topBorder ) // in the top block
			{
				...
				final double lambda = 1.0 - (topBorder - z) / (overlapTop-1);
			...
			else if ( z >= bottomBorder ) // in the bottom block
			{
				...
				final double lambda = 1.0 - (z - bottomBorder) / (overlapBottom-1);
		...
		final ImagePlus imp1 = SolveTools.render( idToFinalModel, idToTileSpec, 0.15 );
		imp1.setTitle( "final" );

		//ImagePlus imp2 = render( idToNewModel, idToTileSpec, 0.15 );
		//imp2.setTitle( "realign" );

		final ImagePlus imp3 = SolveTools.render( idToPreviousModel, idToTileSpec, 0.15 );
